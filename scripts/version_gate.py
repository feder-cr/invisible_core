"""Refuse to publish a core whose shipped content changed while its version did not.

WHY THIS EXISTS
---------------
pip's satisfaction check is NAME + VERSION ONLY, never content. Ship different
bytes under a version string that already exists and every installed copy stays
where it is: pip reports "already satisfied", `pip check` reports "No broken
requirements found", exit 0, and the fix reaches nobody. A public index is
stricter in one direction only (it refuses a filename it has already served) and
completely blind in the other (nothing stops a version that SHOULD have moved
from standing still until the moment of upload, by which point the artifact is
built and the release is half done).

So this is a gate, not a runbook note. It runs before the upload and it exits
non-zero.

WHAT COUNTS AS "CONTENT"
------------------------
The built artifact, not a hand-written list of paths. A path allowlist gets four
things wrong that the artifact gets right:

  * README.md IS content - `readme = "README.md"` embeds the body in METADATA.
  * pyproject.toml is PARTLY content - name, version, dependencies, classifiers
    and requires-python all land in METADATA, so a Requires-Dist change under an
    unchanged version is caught.
  * LICENSE is content - it ships as dist-info/licenses/LICENSE.
  * everything else at the repo root is NOT content, which is the only place the
    ledger can live without being self-referential.

Concretely, two manifests are computed:

  wheel  - path -> sha256 for every member of a freshly built wheel, minus the
           RECORD member (whose bytes are a function of all the others), with
           the versioned dist-info directory normalised to `<dist-info>/`.
  sdist  - path -> sha256 for every regular file in the freshly built tarball,
           with the versioned top-level directory stripped.

The digest of a manifest is sha256 over its sorted "path,hash" lines. Per-file
hashes are used rather than the .whl / .tar.gz bytes on purpose: the digest then
survives a build-backend change to zip framing, member ordering or timestamps,
and a real content change still moves it.

TWO NORMALISATIONS, BOTH OF THEM LOAD-BEARING
---------------------------------------------
A gate that cries wolf gets switched off, so the manifest deliberately does not
digest two classes of difference that are not content:

  * LINE ENDINGS. This repo is checked out on Windows with autocrlf active, so
    19 of the 52 files in today's sdist carry CRLF while the rest carry LF -
    measured, not assumed. The ledger is checked in and meant to be portable,
    so the first CI run on Linux would see every one of those files "change"
    and refuse a release nobody touched. Every file that decodes as UTF-8 and
    holds no NUL byte therefore has its CRLF and lone-CR sequences folded to LF
    before hashing. Genuinely binary payloads are hashed byte for byte. That
    fold is the ONLY blindness this gate accepts, and its width is pinned from
    both sides - see normalise(), which spells out what it deliberately does not
    fold (indentation above all, since it is semantic in Python) and the one
    thing it loses (a CR that is content rather than a line ending).
  * FILES THE BACKEND INJECTS REGARDLESS OF THE INCLUDE LIST. hatchling puts
    .gitignore into the sdist root even though `include` never mentions it
    (measured). Adding a plain .gitignore at the repo root is not a change to
    what users receive, and the first version of this gate refused a release
    over exactly that. SDIST_NONCONTENT names those root entries and drops
    them. It is a short, explicit list of VCS-metadata filenames: anything not
    on it, at the root or anywhere else, still counts.

seal.json GETS NO EXEMPTION, and that is not a contradiction
------------------------------------------------------------
seal.json is both shipped content and the version input, so it needs the exact
rule spelled out:

  * exactly ONE field of it feeds the version, `tag` (_version.py splits it and
    uses the number). Move the tag and the version moves with it, the new
    version is not in the ledger, and this gate passes without anyone doing
    anything. That is the "only seal.json changed" case, and it is a PASS.
  * every OTHER field - five sha256, five build_id, five size, entry_rel,
    omni_sha256, source_commit, upstream_version, playwright.min/max, schema -
    changes the bytes users receive and moves the version not at all. A same-tag
    re-seal is a designed-in workflow (the schema 1 -> 2 correction kept the tag
    at firefox-18; widening the Playwright range is documented as travelling in
    the seal precisely so it needs no consumer release). That is exactly the
    state this gate exists to refuse.

HOW THE LAST RELEASED VERSION IS LEARNED
----------------------------------------
A checked-in ledger, PUBLISHED.json at the repo root. Three options were
weighed:

  index JSON API - authoritative about what was actually accepted, and it can
    even compare sha256 without downloading, but it is ONLINE ONLY and CI may
    run this step with no index access. Kept as an optional cross-check
    (--verify-index), never as the primary source.
  git tags - unavailable: this repo has zero tags today, tags are not fetched by
    a depth-1 CI checkout, they record intent to publish rather than acceptance,
    and they carry no content digest.
  ledger - the only fully offline answer, and the only one that stores a digest.
    Its cost is that it is a claim rather than an observation, so `record` is a
    subcommand of this same script and writes atomically. Nobody hand-edits it.

The ledger MUST stay at the repo root and out of both build targets. Under
src/invisible_core/ it would become wheel content and every release would
invalidate the digest it is certifying.

A MISSING LEDGER IS A FAILURE, NOT A FIRST RELEASE
--------------------------------------------------
The first version of this gate read a missing file as an empty ledger and
printed "this is release 1". That is failing OPEN through the gate's only piece
of memory: a typo in --ledger, a deleted PUBLISHED.json, a shallow checkout that
did not bring the root, a bad merge that dropped it - each one silently disarmed
the gate while a real content change was sitting in the tree. All three were
measured waving a violation through.

So the file must EXIST and PARSE, always, exit 4 otherwise (EXIT_NO_LEDGER,
distinct from every other outcome so CI can tell "the gate has no memory" apart
from "the gate refused"). And an empty ledger is no longer self-evidently a
first release either: `check` refuses it unless the operator passes
--first-release, because release 1 happens exactly once in the life of a project
and is a thing a human states, never a thing a missing file implies. Passing
--first-release does NOT substitute for the file: the flag says "nothing has
been published", the file is what carries that claim into the next run.

--first-release IS NOT TAKEN ON TRUST EITHER
--------------------------------------------
The flag names the one state in which this gate compares nothing at all: an
empty ledger holds no earlier content, so a real content change in the tree is
waved straight through. That was measured - `check --first-release` exited 0
with a modified tree, and `publish --first-release --dry-run` went on to print
the twine upload it authorised. And the operator does not have to be careless to
get there: the recipe is the literal printf that load_ledger prints when the
ledger is missing.

It is also the one claim in this whole file that an outside authority can settle
outright, so the index cross-check is FORCED on that branch (not the optional
--verify-index) and the question asked is the strong one: the index must serve
NO version of this distribution, not merely not this one. An index that cannot
be reached is exit 2 - the gate has no opinion - never exit 0. That costs
nothing that was not already required, since release 1 is run on a machine that
is about to upload to that same index. --index-json-url points the check at the
index a `--repository` upload actually lands on.

A ledger entry that is present but structurally wrong (say, no 'wheel' key) is
exit 2, GATE BROKEN. It is not evidence of a violation, it is evidence that the
gate cannot form an opinion, and the two must never print the same shape. The
same applies to an entry that contradicts ITSELF, and two ways of doing that
were measured slipping past a shape-only check:

  * the version string is what _entry_for matches on and nothing corroborated
    it, so editing released[0].version from 18.0.0 to 17.9.0 made a drifted
    18.0.0 build look unpublished and exit 0. The recorded wheel and sdist
    filenames carry the true version, and they are now cross-checked against it.
  * a digest that does not hash its own file table means the record disagrees
    with itself. It used to pass, because the verdict was computed from the file
    tables while only the digests were printed - garbage digests beside an
    intact table read as "nothing to release". The digests are the verdict now,
    the file tables are the explanation, and an entry where the two disagree is
    refused at load time.

HOW THIS GETS INVOKED (it is not a runbook note)
------------------------------------------------
A gate the operator has to remember is not a gate. Three layers, and none of
them needs anybody to remember anything:

  1. `publish` is a subcommand of THIS script, and it is the only supported way
     to upload. It runs `check` in-process first and returns its exit code
     unchanged on a refusal - twine is never reached. Skipping the gate means
     not using the publish path at all.
  2. .github/workflows/publish.yml holds the index credential in a GitHub
     Environment that only the upload job can see, and that job `needs:` the
     gate job. There is no ordering to get wrong: without a green gate the job
     with the credential never starts.
  3. .githooks/pre-push refuses to push a release tag whose gate is red, so the
     tag that triggers (2) cannot even leave the machine first. Installed by
     `python scripts/install_hooks.py`, which CI re-checks so an uninstalled
     hook is itself a red build.

EXIT CODES
----------
  0  publish allowed
  1  release REFUSED (this is the gate firing)
  2  gate BROKEN (no build backend, the build is not reproducible, or a ledger
     entry is malformed). Never conflated with 1: a toolchain upgrade or a
     corrupt record must not read as a violation.
  3  usage error (bad arguments)
  4  the ledger is MISSING or UNREADABLE - the gate has no memory and therefore
     no opinion. Never conflated with 0: this is the fail-open hole.

USAGE
-----
  python scripts/version_gate.py check              # before the upload
  python scripts/version_gate.py check --verify-index
  python scripts/version_gate.py check --first-release   # only ever once, online
  python scripts/version_gate.py publish            # check, upload, record
  python scripts/version_gate.py record             # after a successful upload
  python scripts/version_gate.py show               # digests only, no verdict

Needs `build` and `hatchling` importable by the running interpreter (both are in
the `dev` extra). It builds into a temporary directory and never writes into the
source tree.
"""
...
def cmd_check(args) -> int:
    ...
    print(f"invisible-core {version}  (seal {info['seal_tag'] or 'unknown'})")
